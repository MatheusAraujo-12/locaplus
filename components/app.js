...
import PriceComparatorPage from './components/PriceComparatorPage';
import ReportsPage from './components/ReportsPage';
import FleetListPage from './components/FleetListPage';
import CarDetailsPage from './components/CarDetailsPage';
import { APP_INSTANCE_ID } from './firebaseConfig';
import {
  auth,
  db,
  onAuthStateChanged,
...
const App = () => {
  ...
  const appInstanceId = APP_INSTANCE_ID;
      ...
      try {
        if (authUser) {
          ...
          if (!snap.exists()) {
            ...
            const invitesQuery = query(invitationsRef, where('email', '==', authUser.email));
            ...
            {[
              ...
              <button
                ...
                className={`w-full text-left px-4 py-2.5 rounded-lg flex items-center gap-3 transition-colors sidebar-link ${
                  currentPage === item.key ? 'active' : 'hover:bg-gray-100 text-gray-600'
                }`}
              >

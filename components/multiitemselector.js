...
const MultiItemSelector = ({ allItems, selectedItems, onSelectionChange, onQuantityChange }) => {
    ...
    const handleAddItem = (item) => {
        onSelectionChange([...selectedItems, { ...item, quantity: 1 }]); // Adiciona com quantidade 1
        setSearchTerm('');
        setIsDropdownOpen(false);
    };
        ...
        <div className="relative md:col-span-2" ref={wrapperRef}>
            ...
            <div className="w-full bg-gray-100 p-2 rounded-lg border focus-within:ring-2 focus-within:ring-blue-500 flex flex-col gap-2 min-h-[48px]">
                <div className="flex flex-col gap-2">
                    {selectedItems.map((item, index) => (
                        <div key={item.id} className="bg-blue-100 text-blue-900 flex items-center justify-between gap-2 px-3 py-2 rounded">
            ...
            {isDropdownOpen && (
                <div className="absolute top-full left-0 right-0 bg-white border mt-1 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                    ...
                    ) : (<div className="p-3 text-gray-500">Nenhum item encontrado.</div>)}
                </div>
            )}
        </div>
    );

;(function () {
  ...
  const isPartItem = (item) =>
    normalizeType(item?.type || "").startsWith("peca");
  ...
  const MaintenanceItemManager = ({ db, basePath, onClose, showAlert }) => {
    ...
    const handleAddItem = async () => {
      ...
      try {
        ...
        showAlert(
          `'${newItemData.name.trim()}' foi adicionado como ${newItemData.type}!`,
          "success"
        );
    ...
    const handleDeleteItem = async (itemId, itemName) => {
      if (!window.confirm(`Tem certeza que deseja apagar o item '${itemName}'?`))
        return;
      try {
    ...
    const handleAddStock = async (itemId) => {
      ...
      try {
        ...
        showAlert(
          `${amountToAdd} unidade(s) adicionada(s) ao estoque!`,
          "success"
        );

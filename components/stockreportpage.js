;(function () {
  ...
  const StockReportPage = ({ userData, db, appInstanceId, showAlert }) => {
    ...
    if (isLoading) {
      return <LoadingSpinner text="Carregando relatório de estoque..." />;
    }

    return (
      <div className="p-4 md:p-6 bg-gray-50 min-h-full">
        {showItemManager && (
          <MaintenanceItemManager
            db={db}
            basePath={basePath}
            showAlert={showAlert}
          ...
          <div className="flex justify-between items-center mb-6 no-print">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="bg-purple-700 text-white font-bold p-2 w-10 h-10 md:px-4 md:w-auto md:h-auto rounded-lg hover:bg-purple-800 flex items-center justify-center transition-all"
              >
              ...
              <button
                ...
                className="bg-gray-700 text-white font-bold p-2 w-10 h-10 md:px-4 md:w-auto md:h-auto rounded-lg hover:bg-gray-800 flex items-center justify-center transition-all"
              >
              ...
              <table className="w-full text-left">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="p-2 md:p-3 text-sm md:text-base font-semibold text-center">
                      Tipo
                    </th>
                    <th className="p-2 md:p-3 text-sm md:text-base font-semibold text-center">
                      Qtd. Estoque
                    </th>
                    <th className="p-2 md:p-3 text-sm md:text-base font-semibold text-right">
                      Preço Unit.
                    </th>
                    <th className="p-2 md:p-3 text-sm md:text-base font-semibold text-right">
                      Valor Estoque
                ...
                <tbody>
                  {items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        {/* FONTES E PADDINGS AJUSTADOS PARA SEREM MENORES NO TELEMÓVEL */}
                        <td className="p-2 md:p-3 text-sm md:text-base font-medium text-gray-800">
                          {item.name}
                        </td>
                        ...
                        <td
                          className={`p-2 md:p-3 text-center font-bold text-base md:text-lg ${
                            isLowStock
                              ? "text-red-600"
                              : "text-gray-800"
                          }`}
                        ...
                        <td className="p-2 md:p-3 text-sm md:text-base text-right">
                          {formatCurrency(item.price)}
                        </td>
                        <td className="p-2 md:p-3 text-sm md:text-base text-right font-semibold">
                          {item.type === "Peça"
  ...
  window.StockReportPage = StockReportPage;
})();

;(function () {
  ...
  const WorkshopManager = ({ db, basePath, onClose, showAlert }) => {
    ...
    const { collection, query, onSnapshot, addDoc, deleteDoc, doc, orderBy } =
      window.firebase;
    ...
    const handleDeleteWorkshop = async (workshopId, workshopName) => {
      if (!confirm(`Tem certeza que deseja apagar a oficina '${workshopName}'?`))
        return;
      try {
        ...
        <div className="bg-white rounded-xl shadow-2xl p-6 md:p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mb-6 p-4 border rounded-lg">
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full bg-blue-800 text-white font-bold py-3 px-5 rounded-lg hover:bg-blue-900 disabled:bg-blue-400 mt-2"
            >
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {shop.address || "Sem endereço"}
                    </p>
                  </div>
                  <button
                    onClick={() =>
                      handleDeleteWorkshop(shop.id, shop.name)
                    }

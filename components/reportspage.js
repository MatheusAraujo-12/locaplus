...
// components/ReportsPage.js - VERSÃO AJUSTADA (COMISSÃO PARA A EMPRESA)
...
const ReportsPage = ({ userData, showAlert, db, appInstanceId }) => {
    ...
    const ExpensePieChart = ({ chartData }) => {
        ...
        if (!chartData || chartData.labels.length === 0) {
            return <p className="text-center text-gray-500 my-4">Não há dados de despesas para exibir o gráfico.</p>;
        }
    ...
    const getDateObject = (rawValue) => {
        ...
        if (typeof rawValue.toDate === 'function') {
            try { return rawValue.toDate(); } catch { return null; }
        }
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            for (const car of carsToProcess) {
                ...
                // commissionPercentage / companyFixedCommission SEMPRE SÃO DA EMPRESA
                ...
                if (netProfit > 0) {
                    if (commissionMode === "fixed" && fixedCommission > 0) {
                        // Empresa recebe um valor fixo, mas nunca mais do que o lucro
                        ...
                        // Empresa recebe % do lucro
                        companyCommission = netProfit * (commissionPercentage / 100);
                        ownerPayout = netProfit - companyCommission;
                    } else {
                        // Sem configuração -> tudo fica para o proprietário
                        companyCommission = 0;
                        ownerPayout = netProfit;
                    }
                } else {
                    // Prejuízo ou zero -> empresa não recebe comissão; prejuízo é do proprietário
                    companyCommission = 0;
                    ownerPayout = netProfit;
                }
                // ---- FIM NOVA LÓGICA ----
    ...
    const handleDownloadPdf = () => {
        ...
        html2canvas(reportElement, { scale: 2, useCORS: true }).then(canvas => {
            ...
            while ((heightLeft -= (pdfHeight - 20)) > 0) {
                pdf.addPage();
                pdf.addImage(imgData, 'PNG', x, y - (imgHeight - heightLeft), imgWidth, imgHeight);
            }
                                    ...
                                    {carReport.netProfit !== 0 && (
                                        <>
                                            <tr className="text-sm text-gray-600">
                                                <td className="pt-2">Parte da Empresa</td>
                                                <td className="pt-2 text-right">
                                                    {formatCurrency(carReport.companyCommission)}
                                                </td>
                                            </tr>
                                            <tr className="text-sm text-gray-600">
                                                <td>Parte do Proprietário</td>
                    ...
                    <div className="mt-8 pt-4 border-t-2">
                        ...
                        <table className="w-full text-left text-lg">
                            <tbody>
                                ...
                                {/* Quando for relatório por proprietário, mostra o split */}
                                {reportData.filterType === 'owner' && reportData.grandTotals.totalNetProfit !== 0 && (
                                    <>
                                        ...
                                        <tr className="text-base">
                                            <td>Total Comissão da Empresa</td>

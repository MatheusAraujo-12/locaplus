;(function () {
  ...
  // components/ReportsPage.js - VERSÃO FINAL EM PT-BR
  ...
  const ReportsPage = ({ userData, showAlert, db, appInstanceId }) => {
    ...
    const [filterType, setFilterType] = useState("all"); // all | single_car | owner
    ...
    const getDateObject = (rawValue) => {
      ...
      if (typeof rawValue === "string") {
        const isoCandidate = rawValue.includes("T")
          ? rawValue
          : `${rawValue}T00:00:00`;
    ...
    // --- SUB-COMPONENTE: GRÁFICO DE DESPESAS POR CATEGORIA ---
    ...
    // --- SUB-COMPONENTE: GRÁFICO COMPARATIVO POR VEÍCULO ---
    ...
    const handleGenerateReport = async () => {
      ...
      try {
        ...
        for (const car of carsToProcess) {
          ...
          // RECEITAS
          const revenuesRef = collection(
            db,
            `${basePath}/cars/${car.id}/revenues`
          );
          ...
          const ownerPayout =
            netProfit > 0 ? netProfit * (commissionPercentage / 100) : 0;
    ...
    const handleDownloadPdf = () => {
      ...
      html2canvas(reportElement, { scale: 2, useCORS: true }).then(
        (canvas) => {
          ...
          const safeTitle = (reportData?.reportTitle || "relatorio").replace(
            /[\s:]/g,
            "_"
          );
          const fileName = `relatório_${safeTitle}_${new Date()
            .toISOString()
            .slice(0, 10)}.pdf`;
          pdf.save(fileName);
        }
    ...
    const handleExportCsv = () => {
      if (!reportData || !reportData.cars) {
        showAlert(
          "Gere um relatório primeiro para exportar os dados.",
          "error"
        );
        return;
      }
      ...
      const fileName = `relatório_detalhado_${new Date()
        .toISOString()
        .slice(0, 10)}.csv`;
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg mb-6 no-print">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <button
              ...
              disabled={
                isLoading || (filterType !== "all" && selectedId === "all")
              }
                    ...
                    reportData.grandTotals.totalNetProfit > 0 && (
                      <>
                        <tr className="text-base border-t-4 border-transparent">
                          <td className="pt-3">
                            Total para {reportData.ownerName}
                          </td>
                          <td className="pt-3 font-bold text-right">
                            {formatCurrency(
                              reportData.grandTotals.totalOwnerPayout
                            )}
                          </td>
                        </tr>

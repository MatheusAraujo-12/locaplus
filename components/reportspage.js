...
const ReportsPage = ({ userData, showAlert, db, appInstanceId }) => {
    ...
    const CarFinancialChart = ({ carReports }) => {
        ...
        return (<div className="w-full mx-auto p-4"><h3 className="text-xl font-bold text-gray-800 mb-4 text-center">Comparativo Financeiro por Veículo</h3><canvas ref={chartRef}></canvas></div>);
    };
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            let carsToProcess = [];
            let reportTitle = '';
            if (filterType === 'all') { carsToProcess = cars; reportTitle = 'Relatório Geral da Frota'; } 
            else if (filterType === 'single_car' && selectedId !== 'all') { carsToProcess = cars.filter(c => c.id === selectedId); reportTitle = `Relatório - ${cars.find(c=>c.id === selectedId)?.name}`; } 
            else if (filterType === 'owner' && selectedId !== 'all') { carsToProcess = cars.filter(c => c.ownerName === selectedId); reportTitle = `Relatório do Proprietário: ${selectedId}`; }
            ...
            for (const car of carsToProcess) {
                ...
                const ownerPayout = netProfit > 0 ? netProfit * (commissionPercentage / 100) : 0;
                const companyCommission = netProfit - ownerPayout;
                carReports.push({ id: car.id, name: car.name, totalRevenue: carRevenue, totalExpense: carExpense, netProfit, ownerPayout, companyCommission, expenses: currentCarExpenses, revenues: currentCarRevenues });
            }
            const expenseByCategory = carReports.flatMap(c => c.expenses).reduce((acc, expense) => { const category = expense.category || 'Outros'; acc[category] = (acc[category] || 0) + Number(expense.cost || 0); return acc; }, {});
            const grandTotals = { totalRevenue: carReports.reduce((sum, rep) => sum + rep.totalRevenue, 0), totalExpense: carReports.reduce((sum, rep) => sum + rep.totalExpense, 0), totalNetProfit: carReports.reduce((sum, rep) => sum + rep.netProfit, 0), totalOwnerPayout: carReports.reduce((sum, rep) => sum + rep.ownerPayout, 0), totalCompanyCommission: carReports.reduce((sum, rep) => sum + rep.companyCommission, 0) };
            setReportData({ reportTitle, period: `${new Date(startDate + 'T00:00:00').toLocaleDateString('pt-BR')} a ${new Date(endDate + 'T00:00:00').toLocaleDateString('pt-BR')}`, cars: carReports, grandTotals, chartData: { labels: Object.keys(expenseByCategory), values: Object.values(expenseByCategory) }, filterType, ownerName: filterType === 'owner' ? selectedId : null });
        } catch (error) { console.error("Erro ao gerar relatório: ", error); showAlert("Ocorreu um erro ao gerar o relatório.", "error");
    ...
    const handleDownloadPdf = () => {
        ...
        html2canvas(reportElement, { scale: 2, useCORS: true }).then(canvas => {
            ...
            const fileName = `relatorio_${reportData.reportTitle.replace(/[\s:]/g, '_')}_${new Date().toISOString().slice(0,10)}.pdf`;
            pdf.save(fileName);
        });
    };
    ...
    const handleExportCsv = () => {
        ...
        const fileName = `relatorio_detalhado_${new Date().toISOString().slice(0,10)}.csv`;
        ...
        <div className="p-4 md:p-6">
            ...
            <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg mb-6 no-print">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                    <div><label className="block text-sm font-medium text-gray-700">Tipo de Relatório</label><select value={filterType} onChange={e => { setFilterType(e.target.value); setSelectedId('all'); }} className="mt-1 w-full bg-gray-100 p-3 rounded-lg border focus:ring-2 focus:ring-blue-500"><option value="all">Toda a Frota</option><option value="single_car">Por Veículo</option><option value="owner">Por Proprietário</option></select></div>
                    {filterType === 'single_car' && <div><label className="block text-sm font-medium text-gray-700">Veículo</label><select value={selectedId} onChange={e => setSelectedId(e.target.value)} className="mt-1 w-full bg-gray-100 p-3 rounded-lg border focus:ring-2 focus:ring-blue-500"><option value="all" disabled>Selecione um veículo</option>{cars.map(car => <option key={car.id} value={car.id}>{car.name}</option>)}</select></div>}
                    {filterType === 'owner' && <div><label className="block text-sm font-medium text-gray-700">Proprietário</label><select value={selectedId} onChange={e => setSelectedId(e.target.value)} className="mt-1 w-full bg-gray-100 p-3 rounded-lg border focus:ring-2 focus:ring-blue-500"><option value="all" disabled>Selecione um proprietário</option>{uniqueOwners.map(owner => <option key={owner} value={owner}>{owner}</option>)}</select></div>}
                    <div className={filterType !== 'all' ? '' : 'md:col-start-3'}><label className="block text-sm font-medium text-gray-700">Data de Início</label><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="mt-1 w-full bg-gray-100 p-3 rounded-lg border focus:ring-2 focus:ring-blue-500"/></div>
                    <div><label className="block text-sm font-medium text-gray-700">Data de Fim</label><input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="mt-1 w-full bg-gray-100 p-3 rounded-lg border focus:ring-2 focus:ring-blue-500"/></div>
                    <button onClick={handleGenerateReport} disabled={isLoading || (filterType !== 'all' && selectedId === 'all')} className="w-full bg-blue-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed">{isLoading ? 'A Gerar...' : 'Gerar Relatório'}</button>
                </div>
            </div>
            {isLoading && <LoadingSpinner text="A gerar relatório..." />}
            {reportData && (
                <div id="pdf-layout" className="bg-white p-6 md:p-8 rounded-xl shadow-lg">
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="flex gap-2 no-print">
                            <button onClick={handleDownloadPdf} className="download-button bg-gray-200 text-gray-800 font-bold p-2 w-10 h-10 md:px-4 md:w-auto md:h-auto rounded-lg hover:bg-gray-300 flex items-center justify-center transition-all"><i className="fas fa-file-pdf"></i><span className="hidden md:inline md:ml-2">PDF</span></button>
                            <button onClick={handleExportCsv} className="bg-green-200 text-green-800 font-bold p-2 w-10 h-10 md:px-4 md:w-auto md:h-auto rounded-lg hover:bg-green-300 flex items-center justify-center transition-all"><i className="fas fa-file-csv"></i><span className="hidden md:inline md:ml-2">CSV</span></button>
                        </div>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div className="bg-green-100 p-6 rounded-xl shadow-lg text-center"><p className="text-sm font-bold text-green-800">Receita Total</p><p className="text-3xl md:text-4xl font-extrabold text-green-900 mt-2">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(reportData.grandTotals.totalRevenue)}</p></div>
                        <div className="bg-red-100 p-6 rounded-xl shadow-lg text-center"><p className="text-sm font-bold text-red-800">Despesa Total</p><p className="text-3xl md:text-4xl font-extrabold text-red-900 mt-2">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(reportData.grandTotals.totalExpense)}</p></div>
                        <div className="bg-blue-100 p-6 rounded-xl shadow-lg text-center"><p className="text-sm font-bold text-blue-800">Lucro Líquido Total</p><p className="text-3xl md:text-4xl font-extrabold text-blue-900 mt-2">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(reportData.grandTotals.totalNetProfit)}</p></div>
                    </div>
                    ...
                    <div className="mt-8 pt-4 border-t-2">
                        ...
                        <table className="w-full text-left text-lg"><tbody><tr className="border-b"><td className="p-2">Total de Receitas</td><td className="p-2 font-bold text-green-600 text-right">R$ {reportData.grandTotals.totalRevenue.toFixed(2)}</td></tr><tr className="border-b"><td className="p-2">Total de Despesas</td><td className="p-2 font-bold text-red-600 text-right">- R$ {reportData.grandTotals.totalExpense.toFixed(2)}</td></tr><tr className="bg-gray-100 font-bold"><td className="p-3 text-xl">Lucro Líquido Total</td><td className={`p-3 text-xl text-right ${reportData.grandTotals.totalNetProfit >= 0 ? 'text-blue-700' : 'text-orange-700'}`}>R$ {reportData.grandTotals.totalNetProfit.toFixed(2)}</td></tr>
                        {reportData.filterType === 'owner' && reportData.grandTotals.totalNetProfit > 0 && (<> <tr className="text-base border-t-4 border-transparent"><td className="pt-3">↳ Total para {reportData.ownerName}</td><td className="pt-3 font-bold text-right">R$ {reportData.grandTotals.totalOwnerPayout.toFixed(2)}</td></tr><tr className="text-base"><td className="">↳ Total Comissão da Empresa</td><td className="text-right font-bold">R$ {reportData.grandTotals.totalCompanyCommission.toFixed(2)}</td></tr></>)}
                        </tbody></table>
                    </div>

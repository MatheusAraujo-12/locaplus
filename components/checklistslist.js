          ...
          {checklists.map((cl) => (
            <div key={cl.id} className="p-4 bg-gray-50 rounded-lg border">
                  ...
                  <p className="font-bold">
                    {cl.date && cl.date.toDate
                      ? new Intl.DateTimeFormat("pt-BR", {
                          dateStyle: "full",
                          timeStyle: "short",
                        }).format(cl.date.toDate())
                      : cl.date
                      ? new Intl.DateTimeFormat("pt-BR", {
                          dateStyle: "full",
                          timeStyle: "short",
                        }).format(new Date(cl.date))
                      : "Data inválida"}
                  </p>
                </div>
                <div className="flex items-center gap-4 self-end sm:self-center">
                  <button
                    onClick={() => onView && onView(cl)}
                  ...
                  {isAdmin && (
                    <button
                      onClick={() =>
                        onRequestDelete && onRequestDelete(cl)
                      }
              ...
              <p className="text-sm text-gray-600 mt-2">
                por: {cl.checkedBy} @ {cl.mileageAtCheck} km
              </p>
              {cl.driverName && (
                <p className="text-sm text-gray-600">
                  Motorista: <strong>{cl.driverName}</strong>
                </p>
              )}
            </div>
          ))}
...
window.ChecklistsList = ChecklistsList;
